...
- 처리 과정 (아래 1~6번이 **한 층**이고, 같은 구조의 층을 여러 개 쌓습니다 — 원논문은 6개)
...
#### 왜 층을 여러 개 쌓는가 (그리고 왜 6이 정답은 아닌가)

- ⚠️ **6은 마법의 숫자가 아닙니다**
  - 원논문(*Attention Is All You Need*)이 번역 과제에 맞춰 고른 **하이퍼파라미터(N=6)** 일 뿐입니다
  - 층 수는 모델 설계자가 정하는 값이고, 데이터·컴퓨팅 규모가 커지면서 함께 커졌습니다

  | 모델 | Encoder/Decoder 층 수 |
  |---|---|
  | Transformer (원논문) | 6 |
  | BERT-base / BERT-large | 12 / 24 |
  | GPT-3 | 96 |
  | 최신 대형 LLM | 60~130+ |

  - 그러니 외워야 할 건 "6번"이 아니라 **"여러 층을 쌓는다"** 는 사실입니다

- ⚠️ **"6번 반복"이라는 말의 흔한 오해**
  ```
  ❌ 오해:  하나의 층을 for 문처럼 6번 통과 (가중치 같음)
  ✅ 실제:  서로 다른 6개 층을 차례로 통과 (층마다 가중치가 전부 다름)
  ```
  - 층 하나에 W_Q/W_K/W_V/W_O + FFN 가중치가 들어 있고, 6층이면 이 세트가 **6벌** 있습니다
  - 가중치가 층마다 다르기 때문에 **층마다 다른 역할을 학습**할 수 있는 것입니다

- **이유 ①: 어텐션 한 층은 "정보 섞기 1회"밖에 못 합니다**
  - Self-Attention 한 번은 각 단어가 다른 단어들을 **한 번 훑어오는** 것으로 끝납니다
  - 그런데 언어 이해에는 여러 단계를 거쳐야 하는 관계가 있습니다
    > "**철수**가 사과를 샀다. **그것**은 **그의** 첫 선물이었다."
    - 1홉: "그것" → "사과"를 찾음
    - 2홉: "사과"가 "선물"이라는 걸 알아야 "그의"가 "철수"인지 판단 가능
    - 3홉: 문장 전체가 "선물 구매" 상황이라는 이해로 통합
  - 이걸 한 층에서 동시에 할 수 없습니다. **1층이 찾아낸 결과 위에서 2층이 다시 계산**해야
    다단계 추론이 됩니다 → 층을 쌓는 건 **참조의 참조**를 가능하게 하는 것

- **이유 ②: 층마다 하는 일이 다릅니다** (BERT 분석 연구들에서 반복 확인된 경향)
  - 하위층: 품사, 어순, 인접 단어 → **문법적** 관계
  - 중간층: 구문 구조, 주어-서술어 관계 → **구조적** 관계
  - 상위층: 의미, 지칭 해소, 과제별 추상 개념 → **의미적** 관계
  - 즉 같은 작업의 반복이 아니라 **문법 → 구조 → 의미로 올라가는 계단**입니다

- **그런데 왜 무한히 늘리지 않나**
  - **수익 체감**: 층을 늘리면 성능이 오르지만 어느 지점부터 완만해집니다
  - **비용**: 층수만큼 파라미터·연산·메모리가 선형으로 증가합니다
  - **학습 안정성**: 깊어지면 학습이 어려워집니다 — 이걸 막는 장치가 위 4·6번의
    **잔차연결(Add)** 과 **정규화(Norm)** 입니다. 이 둘이 없으면 6층도 제대로 학습되지 않습니다

- **한 줄 비유**
  > 문장을 한 번 읽고 덮는 게 아니라, **관점이 다른 여러 사람이 차례로 읽으면서**
  > 앞사람의 이해 위에 자기 이해를 덧붙이는 것입니다.
  > 아래쪽 사람은 문법을 보고, 위쪽 사람은 의미를 봅니다.
...
- 처리과정 (아래 1~8번이 **한 층**이고, 같은 구조의 층을 여러 개 쌓습니다 — 원논문은 6개)
  ...
  - **3. Masked Multi-Head Self-Attention**
    ...
    - ⚠️ 마스킹이 **언제 필요한지** 구분하세요
      - **학습 시**: 정답 문장 전체를 한 번에 넣으므로 **반드시 필요**
      - **추론 시**: 한 토큰씩 생성하므로 **가릴 미래가 아예 없습니다** → 사실상 무의미
      - 즉 마스킹은 아키텍처의 본질이 아니라 **"병렬 처리를 순차처럼 보이게 만드는 장치"** 입니다.
        추론은 원래 순차라서 마스크가 하던 일을 **시간이 대신**합니다
...
- Scaling law: 규모를 키울 수록 더 좋은 성능을 냄
  - 더 크게 학습 -> 더 많은 인프라(GPU, 전기 등) 필요 
  - 친칠라의 Scaling law: 구글 실험. 모델 크기가 무조건 큰게 좋은게 아니라 **적절한 모델 크기 + 충분한 데이터가 더 나은 성능**
  - DeekSeek 케이스: 똑똑한 모델로 작은 모델 만들기
    - 더 적은 비용으로 더 높은 성능을 낼 수 있음을 증명
    - 증류 기법 사용: 
      - 좋은 모델에게 질문하여 그 답을 학습에 쓰는 방법
      - DeepSeek-R1이라는 큰 LLM 모델을 이용하여 경량 모델을 뽑아냄 
    - Soft label: One-hot 형식의 확정적 레이블이 아니라 모델이 예측한 확률분포를 학습함
- MoE(Mixture of Experts): 
  - 요청에 적합한 일부 파라미터만 사용하는 기법 -> 더 적은 크기의 모델 사용 가능
  - 트렌스포머의 FFN을 여러개 만들고 그 앞에 라우터를 붙임 
    -> 라우터가 확률이 높은 FFN으로 라우팅 -> 학습이 진행되면서 각 FFN이 Expert로 성장함 
  - FFN이 늘어나므로 전체 파라미터 크기는 커지나 한 질문에 활성화되는 파라미터는 훨씬 작음 
    (예: Qwen3.5-397B-A17B)  
